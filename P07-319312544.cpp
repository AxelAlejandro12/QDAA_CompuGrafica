...
int main()
{
	...
	spotLights[2] = SpotLight(
		...
		1.0f, 0.7f, 1.8f,   // atenuación constante, lineal y cuadrática
	...
	while (!mainWindow.getShouldClose())
	{
		...
		// Faro azul del coche: se mueve con el modelo Kitt
		glm::vec3 carPosition = glm::vec3(-2.5f, 0.5f, 0.0f);
		// posición del faro respecto al coche
		glm::vec3 faroPos = carPosition + glm::vec3(2.0f, 0.5f, 11.5f); // ligeramente al frente y arriba
		// dirección del faro (hacia adelante en X)
		glm::vec3 faroDir = glm::vec3(-1.0f, -0.1f, 0.0f);
		// actualizar posición y dirección del faro azul
		spotLights[2].SetFlash(faroPos, faroDir);

		glm::vec3 helPosition = glm::vec3(0.0f + mainWindow.getmuevex(), 5.0f, 6.0f);
		glm::vec3 faroPos2 = helPosition + glm::vec3(1.77f, -0.1f, 0.0f); // ligeramente al frente y arriba
		...
		//Antorcha
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f, -0.7f, -8.0));
