...
float avanceoffset;
float intervaloCambioNum = 100.0f;
float angulodeala = 0.0f;
float velAla = 5.0f;
bool posAla = true;
float delanteDragon = 0.0f;
float finDragon = 10000.0f;
bool delDragon = true;
static float tiempoAcum = 0.0f;
//1
static float tiempo1 = 0.0f;
static bool num1 = true;
float intervalo = 40.0f;
...
int main()
{
	...
	AlaI_M = Model();
	AlaI_M.LoadModel("Models/alader.obj");
	AlaD_M = Model();
	AlaD_M.LoadModel("Models/alaizq.obj");

	Arco_M = Model();
	Arco_M.LoadModel("Models/arco.fbx");
	PuertaI_M = Model();
	PuertaI_M.LoadModel("Models/puertai.obj");
	PuertaD_M = Model();
	PuertaD_M.LoadModel("Models/puertad.obj");
	Letrero_M = Model();
	...
	while (!mainWindow.getShouldClose())
	{
		...
		*/
		...
		// --- Aleteo de alas ---
		if (posAla) {
			angulodeala += velAla * deltaTime;
			if (angulodeala >= 45.0f) posAla = false;
		}
		else {
			angulodeala -= velAla * deltaTime;
			if (angulodeala <= -45.0f) posAla = true;
		}

		// --- Movimiento del dragón ---
		if (delDragon) {
			delanteDragon += 5.0f * deltaTime;
			dragonavance -= avanceoffset * deltaTime;
			if (delanteDragon >= finDragon) delDragon = false;
		}
		else {
			delanteDragon -= 5.0f * deltaTime;
			dragonavance += avanceoffset * deltaTime;
			if (delanteDragon <= -finDragon) delDragon = true;
		}

		tiempo1 += deltaTime;
		if (tiempo1 >= intervalo) {
			num1 = !num1;
			tiempo1 = 0.0f;
		}
		...
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f + dragonavance, 8.0f + 3.0f * sin(glm::radians(angulovaria)), 0.0f));
		...
		//agregar ala izquierda
		model=modelaux;
		model = glm::translate(model, glm::vec3(0.0f, 3.2222f, 0.65465f));
		//model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::rotate(model, glm::radians(angulodeala), glm::vec3(0.0f, 0.0f, 1.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		AlaI_M.RenderModel();

		//agregar ala derecha
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, 3.2124f, -0.26675f));
		//model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::rotate(model, glm::radians(angulodeala), glm::vec3(0.0f, 0.0f, 1.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		AlaD_M.RenderModel();
		...
		//Puerta izquierda
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.37031f, 0.0f, 0.01266f));
		model = glm::rotate(model, glm::radians(mainWindow.getpuertagiro()), glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		PuertaI_M.RenderModel();

		//Puerta derecha
		model = modelaux;
		model = glm::translate(model, glm::vec3(-0.37031f-mainWindow.getpuertades(), 0.0f, 0.01266f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		toffset2 = glm::vec2(toffsetletrerou, toffsetletrerov);

		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, 1.6327f, 0.090944f));
		model = glm::scale(model, glm::vec3(1.0f, 0.4f, 0.0f));
		...
		//Letrero
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, 1.6327f, 0.080944f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		*/
		...
		tiempoAcum += deltaTime;

		// Cuando se alcanza el intervalo, se incrementa el número
		if (tiempoAcum >= intervaloCambioNum) {
			toffsetnumerocambiau += 0.25f;
			if (toffsetnumerocambiau > 1.0f)
				toffsetnumerocambiau = 0.0f;
			tiempoAcum = 0.0f; // reinicia el temporizador
		}
