...
// Función para guardar fotogramas clave en un archivo
void saveKeyFramesToFile() {
	// Abrir el fichero en modo truncado para sobrescribir el contenido existente
	std::ofstream file("keyframes_avion.txt", std::ios::trunc);
	...
	// Escribe el FrameIndex en el archivo para saber cuántos frames leer luego
	file << "FrameIndex = " << FrameIndex << ";\n";

	// Escribe el contenido de cada KeyFrame
	for (int i = 0; i < FrameIndex; ++i) {
		file << "KeyFrame[" << i << "].movAvion_x = " << KeyFrame[i].movAvion_x << "f;\n";
		file << "KeyFrame[" << i << "].movAvion_y = " << KeyFrame[i].movAvion_y << "f;\n";
		file << "KeyFrame[" << i << "].giroAvion = " << KeyFrame[i].giroAvion << "f;\n\n";
	}

	file.close();
	std::cout << "\n? Keyframes guardados en keyframes_avion.txt\n";
}
...
int main()
{
	...
	Cohete_M = Model();
	Cohete_M.LoadModel("Models/cohete.obj");
	...
	KeyFrame[2].movAvion_y = 8.0f;
	KeyFrame[2].giroAvion = 0;

	KeyFrame[3].movAvion_x = 4.0f;
	KeyFrame[3].movAvion_y = 12.0f;
	KeyFrame[3].giroAvion = -45.0;

	KeyFrame[4].movAvion_x = 8.0f;
	KeyFrame[4].movAvion_y = 16.0f;
	KeyFrame[4].giroAvion = -90.0f;

	KeyFrame[5].movAvion_x = 12.0f;
	KeyFrame[5].movAvion_y = 16.0f;
	KeyFrame[5].giroAvion = -90.0f;
