...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	if (key == GLFW_KEY_E) {      // Avanzar
		theWindow->posAutoZ -= 0.5f;  // mover hacia adelante
	}
	if (key == GLFW_KEY_R) {      // Retroceder
		theWindow->posAutoZ += 0.5f;  // mover hacia atrás
	}
	...
	*/
	...
	if (key == GLFW_KEY_F)
	{
		if (theWindow->articulacion1 > -45)   // mientras no pase el límite negativo
		{
	...
	if (key == GLFW_KEY_G)
	{
		if (theWindow->articulacion1 < 0)    // mientras no pase el límite positivo
		{
	...
	if (key == GLFW_KEY_E)
	{
		theWindow->articulacion2 -= 10.0; // avanza libremente
	}

	if (key == GLFW_KEY_R)
	{
		theWindow->articulacion2 += 10.0; // retrocede libremente
	}


	if (key == GLFW_KEY_E)
	{
		if (theWindow->articulacion3 > 20)
		{
		}
		else
		{
